...
unsigned detectEyes(cv::Mat& resultFrame, cv::CascadeClassifier& faceCascade, cv::CascadeClassifier& eyeCascade, cv::CascadeClassifier& nikeCascade) {
    ...
    faceCascade.detectMultiScale(grayscale, faces, 1.1, 4, 0 | cv::CASCADE_SCALE_IMAGE, cv::Size(120, 80)); // detect faces in frame
    
    if (faces.size() == 0) return 0; // any face was detected
    ...
    for (int j(0); j < faces.size(); ++j) {
        ...
        eyeCascade.detectMultiScale(face, eyes, 1.05, 8, 0 | cv::CASCADE_SCALE_IMAGE, cv::Size(20, 20)); // detect eye in faces
        rectangle(resultFrame, faces[j].tl(), faces[j].br(), cv::Scalar(255, 0, 0), 2); //Draw Faces Border

        if (eyes.size() > (faces.size() * 2)) return faces.size(); // too many eyes detected
        
        for (cv::Rect& eye : eyes) {

            rectangle(resultFrame, 
                      faces[j].tl() + eye.tl(),
                      faces[j].tl() + eye.br(), 
                      cv::Scalar(0, 255, 0), 2); //Draw eyes border
        }

        for (int i(0); i < eyes.size(); ++i) {

            cv::Mat eye = face(eyes[i]);
            cv::equalizeHist(eye, eye); //enhance eye contrast
...
int main() {
    ...
    if (!faceCascade.load("./ressources/haarcascade_frontalface_alt.xml")) {
        std::cerr << "Could not load face detector." << std::endl;
        return -1;
    }
    if (!eyeCascade.load("./ressources/haarcascade_eye_tree_eyeglasses.xml")) {
